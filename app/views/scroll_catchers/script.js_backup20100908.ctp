//Get the initial position of scrollbar.
...
var currentTime = 0;
var now = null;
//var userId = <?php echo hash("md5", strtotime("now").$scroll_catcher["ScrollCatcher"]["access_code"]); ?>;

//Speed limit (pixel/second), if scroll speed exceeds this number, an alert box will show.
var specifiedSpeed = <?php echo $scroll_catcher["ScrollCatcher"]["speed"]; ?>;

//Speed limit (pixel/second), if scroll distance exceeds this number, an alert box will show.
...
function loadDialogue(width, height, iframe_url) {
  if (jQuery("#catcher_container").length == 0) {
      var appendText = "<div id='catcher_container' style='display:none'>\n\
                                              <iframe width='" + width + "' height='" + height + "' src='" + iframe_url + "'>\n\
                                              </iframe>\n\
                                          </div>";
      jQuery("body").append(appendText);
      jQuery("#catcher_container").dialog({modal: true, width: width + 40, height: height + 70, autoOpen: false, close: function () {if (repeat) {jQuery(window).bind("scroll", monitorScroll)}}});
      adjustDialogTimeout = setTimeout(function() {
        x = (document.body.clientWidth - width - 40)/2;
        y = (document.body.clientHeight - height - 70)/2;
        ...
        clearTimeout(adjustDialogTimeout);
      }, 1000);
  }
...
function loadIFrame(iframe_url) {
	if (jQuery("#catcher_container").length == 0) {
		var appendText = "<div id='catcher_container' style='display:none'>\n\
												<a href='" + iframe_url + "'>Show Catcher iFrame</a>\n\
											</div>";
		jQuery("body").append(appendText);
